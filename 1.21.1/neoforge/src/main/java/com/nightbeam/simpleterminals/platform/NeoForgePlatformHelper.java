...
public class NeoForgePlatformHelper implements IPlatformHelper {
    ...
    private static Optional<Container> getItemHandlerContainer(Level level, BlockPos pos, BlockEntity blockEntity, Direction accessSide) {
        Optional<Container> sided = resolveItemHandler(level.getCapability(Capabilities.ItemHandler.BLOCK, pos, accessSide), blockEntity);
        if (sided.isPresent()) {
            return sided;
        }
        Optional<Container> unsided = resolveItemHandler(level.getCapability(Capabilities.ItemHandler.BLOCK, pos, null), blockEntity);
        if (unsided.isPresent()) {
            return unsided;
        }
        for (Direction direction : Direction.values()) {
            Optional<Container> directional = resolveItemHandler(level.getCapability(Capabilities.ItemHandler.BLOCK, pos, direction), blockEntity);
    ...
    private static Optional<Container> resolveItemHandler(IItemHandler handler, BlockEntity owner) {
