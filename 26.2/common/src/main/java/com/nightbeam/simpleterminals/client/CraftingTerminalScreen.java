...
public class CraftingTerminalScreen extends AbstractContainerScreen<CraftingTerminalMenu> {
    ...
    public CraftingTerminalScreen(CraftingTerminalMenu menu, Inventory playerInventory, Component title) {
        super(menu, playerInventory, title, 304, 240);
        inventoryLabelY = 147;
    }
    ...
    public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (searchBox != null && searchBox.getValue().isEmpty() && !searchBox.isFocused()) {
            graphics.text(font, "Search", leftPos + 9, topPos + 20, 0xFF6E7681, false);
        }
        extractTooltip(graphics, mouseX, mouseY);
    }

    private void drawTerminalBackground(GuiGraphicsExtractor graphics) {
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        graphics.text(font, title, titleLabelX, titleLabelY, 0xE6EDF3, false);
        graphics.text(font, playerInventoryTitle, inventoryLabelX, inventoryLabelY, 0xE6EDF3, false);
        graphics.text(font, Component.literal("Crafting"), 202, 40, 0xE6EDF3, false);
    }

    @Override
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (menu.getMaxOffsetRows() > 0 && scrollY != 0.0D) {
            sendScroll(menu.getOffsetRows() - (int) Math.signum(scrollY));
            return true;
        }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (event.buttonInfo().button() == 0 && isInScrollbar(event.x(), event.y())) {
            scrolling = true;
            scrollTo(event.y());
            return true;
        }
